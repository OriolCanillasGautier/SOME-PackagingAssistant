...
class AdaptiveWrapper:
    ...
    def _extract_vertices_from_geometry(self) -> List[Tuple[float, float, float]]:
        ...
        try:
            ...
            if hasattr(self.original_geometry, 'faces'):
                # Limitar a màxim 1000 vèrtexs per evitar sobrecàrrega
                max_vertices = 1000
                vertex_count = 0
                ...
                # Limitar també si venen directament
                all_vertices = self.original_geometry.vertices[:1000]
            else:
            ...
            # Eliminar duplicats amb una aproximació més ràpida
            unique_vertices = []
            for vertex in all_vertices:
                vertex_tuple = tuple(float(coord) for coord in vertex[:3])
                # Usar una aproximació més ràpida per vèrtexs únics
                is_duplicate = False
                for existing in unique_vertices:
                    if abs(vertex_tuple[0] - existing[0]) < 1e-3 and \
                       abs(vertex_tuple[1] - existing[1]) < 1e-3 and \
                       abs(vertex_tuple[2] - existing[2]) < 1e-3:
                        is_duplicate = True
                        break
                ...
                # Limitar a 100 vèrtexs únics màxim
                if len(unique_vertices) >= 100:
                    break
    ...
    def _extract_unique_vertices_from_faces(self, faces: List[List[Tuple[float, float, float]]]) -> List[Tuple[float, float, float]]:
        ...
        for face in faces:
            for vertex in face:
                # Usar aproximació ràpida
                is_duplicate = False
                for existing in unique_vertices:
                    if abs(vertex[0] - existing[0]) < 1e-3 and \
                       abs(vertex[1] - existing[1]) < 1e-3 and \
                       abs(vertex[2] - existing[2]) < 1e-3:
                        is_duplicate = True
                        break
