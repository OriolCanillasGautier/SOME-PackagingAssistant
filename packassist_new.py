...
class PackAssistOriginalGUI:
    ...
    def create_simplify_tab(self):
        ...
        ttk.Radiobutton(method_frame, text="PyMeshLab Quadric (Recomanat)", 
                       variable=self.simplify_method, value="quadric_advanced").pack(side=tk.LEFT, padx=(0, 15))
        ttk.Radiobutton(method_frame, text="Trimesh Alternatiu", 
                       variable=self.simplify_method, value="trimesh_fallback").pack(side=tk.LEFT)
    ...
    def _update_interface_after_import(self, filepath):
        ...
        self.update_mesh_info(self.original_info_text, self.original_mesh_info)
    ...
    def update_mesh_info(self, text_widget, mesh_info):
        """Actualitza la informació de la malla"""
        ...
        if mesh_info:
            ...
            if 'dimensions' in mesh_info:
                ...
                info += f"   • Dimensions:\n"
                info += f"     ↔ Amplada: {dims.get('width', 0):.2f} mm\n"
                info += f"     ↕ Profunditat: {dims.get('depth', 0):.2f} mm\n"
                info += f"     ⬆ Altura: {dims.get('height', 0):.2f} mm\n"
    ...
    def visualize_3d_with_options(self):
        """Visualització 3D dels resultats d'optimització"""
        ...
        try:
            # Usar el nou sistema de visualització si està disponible
            if MODULES_AVAILABLE and hasattr(self, 'visualizer_3d'):
                print("🎯 Visualitzant resultats amb nou sistema...")
                self.visualizer_3d.show_packing_results(
                    mesh=self.simplified_mesh if hasattr(self, 'simplified_mesh') and self.simplified_mesh else self.original_mesh,
                    positions=self.optimization_results.get('positions', []),
                    rotations=self.optimization_results.get('rotations', []),
                    container_dims=self.optimization_results.get('box_dims', {}),
                    title="Resultats d'Optimització"
                )
            else:
                messagebox.showinfo("Info", "Sistema de visualització 3D no disponible. Instal·la PyVista per veure els resultats en 3D.")
    ...
    def _finish_optimization(self, result):
        ...
        if result:
            ...
            # Activar botons de visualització i exportació si existeixen
            if hasattr(self, 'visualize_3d_btn'):
                self.visualize_3d_btn.config(state='normal')
            if hasattr(self, 'export_result_btn'):
